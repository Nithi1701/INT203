...
const selectLevel = (level) => {
    const levels = level 
    if(levels=='easy') {
      setTimeout(() => { showNumber.value = ''; status.value = 'ans' }, 4500)}
    else if (levels=='normal') {
      setTimeout(() => { showNumber.value = ''; status.value = 'ans' }, 2500)}
    else if (levels=='hard') {
    setTimeout(() => { showNumber.value = ''; status.value = 'ans' }, 1500)}
};
    ...
    <div class="title" v-if="status == `none`">
      ...
      <div class="button">
        <button class="btn btn-warning btn-lg active" @click="generateNumber">Start</button>
          ...
          <div>
<button type="button" class="btn btn-outline-success"  @click="selectLevel('easy')"  style="margin-right: 10px;">Easy</button> 
<button type="button" class="btn btn-outline-warning"  @click="selectLevel('normal')"  style="margin-right: 10px;">Normal</button>
<button type="button" class="btn btn-outline-danger"   @click="selectLevel('hard')" style="margin-right: 10px;">Hard</button>
</div>
    </div>
